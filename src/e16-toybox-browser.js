...
function createCapabilityVisual(scene) {
  ...
  const targetMarker = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0xe8f3f0, transparent: true, opacity: 0.72 }),
  );
  ...
  const grabMarker = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0xf6d15c, transparent: true, opacity: 0.88 }),
  );
...
async function main() {
  ...
  canvas.addEventListener('wheel', (event) => {
    if (!capabilityHeld) return;
    event.preventDefault();
    desiredReach = updateCapabilityReach(desiredReach, event.deltaY);
  }, { passive: false });
  ...
  phaseEl.textContent = 'E16.2a · DONOR + PHYSICAL CAPABILITY YARD';
  if (controlsEl) {
    controlsEl.innerHTML = '<strong>WASD</strong> move · <strong>Space</strong> jump · <strong>Shift</strong> sprint · <strong>RMB drag</strong> camera';
  }
  if (secondaryEl) {
    secondaryEl.innerHTML = '<strong>LMB hold</strong> reach / earn grab · <strong>LMB + wheel</strong> retract / extend · <strong>release LMB</strong> let go · <strong>R</strong> reset · <strong>H</strong> telemetry';
  }
  debugLabels.external.textContent = 'desired reach / error';
  ...
  statusEl.textContent = `E16.2a experimental · red = Donor carrier · gold = physical core · green = capability · ${baseCounts.dynamicCount + toyboxCounts.dynamicBodies + 2} dynamic bodies`;
  ...
  function physicsTick(dt) {
    ...
    if (capabilityHeld) capabilityDirection = [...basis.forward];

    const reach = capabilityHeld ? desiredReach : E16_CAPABILITY_LIMITS.restReach;
    character.setOrganTargetOffset(horizontalOrganTargetOffset(capabilityDirection, reach));
