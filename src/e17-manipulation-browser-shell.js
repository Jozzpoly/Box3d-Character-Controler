...
export async function startE17ManipulationBrowser({
  createCharacter,
  loadingText = 'Loading Box3D…',
  phaseText,
  secondaryHtml,
  statusText,
  failureLabel,
  ...
  const character = createCharacter(b3, playground.world, {
  ...
  if (secondaryEl) secondaryEl.innerHTML = secondaryHtml;
  ...
  statusEl.textContent = typeof statusText === 'function'
    ? statusText({ baseCounts, toyboxCounts, character })
    : statusText;
  ...
  return { character, playground, toybox };
}

export function reportE17BrowserFailure(label, error) {
  console.error(error);
  const statusEl = document.querySelector('#status');
  if (statusEl) statusEl.textContent = `${label} failed: ${error?.message ?? error}`;
}
