...
function createDynamicBox(world, { position, half, mass, friction = 0.5 }) {
  const bd = b3.b3DefaultBodyDef();
...
class LockedSupportTranslationOrganism {
  constructor(world) {
    ...
    const carriagePosition = [0, ankleY, 0];
    const torsoPosition = [0, ankleY + torsoHalf[1], 0];
    ...
    this.reactionBody = carriage.body;
    ...
    this.mass = this.bodies.reduce((sum, item) => sum + item.mass, 0);
    ...
    this.slider = b3.b3CreatePrismaticJoint(world, slider);
  ...
  sync() {
    ...
    this.torsoCom = bodyCom(this.torso);
...
function wrapReference(world) {
  ...
  return {
    ...
    get mass() { return base.footMass + base.torsoMass; },
    get kp() { return base.kp; },
    get kd() { return base.kd; },
    get maxTorque() { return base.maxTorque; },
    ...
    set lastBalanceTorque(value) { base.lastBalanceTorque = value; },
    sync() { base._sync(); },
    ...
    for (let i = 0; i < b3.getNumContacts(buffer); i++) {
      ...
      for (let m = 0; m < contact.manifoldCount; m++) {
        ...
        for (let p = 0; p < manifold.pointCount; p++) {
          ...
          if (Math.abs(point.normalImpulse ?? 0) > LOAD_EPS || Math.abs(point.totalNormalImpulse ?? 0) > LOAD_EPS) {
            loaded += 1;
          }
        }
      }
...
function wholeBodyState(organism) {
  ...
  for (const item of organism.bodies) {
    const com = bodyCom(item.body);
    const v = bodyVelocity(item.body);
    mass += item.mass;
    for (let axis = 0; axis < 3; axis++) {
      pos[axis] += item.mass * com[axis];
...
function targetedPreStep(organism, targetTilt, supportReactive) {
  ...
  const torque = clamp(requested, -organism.maxTorque, organism.maxTorque);
...
function runCase({ kind, direction }) {
  ...
  const organism = kind === 'reference'
    ? wrapReference(world)
    : new LockedSupportTranslationOrganism(world);
  ...
  function step({ movePlatform = false, targetTilt = 0, collectRamp = false } = {}) {
    ...
    const commandedTilt = movePlatform
      ...
      : targetTilt;
    targetedPreStep(organism, commandedTilt, signal.reactive);
    ...
    if (collectRamp) {
      if (!signal.reactive) rampSupportLossFrames += 1;
      rampHorizontalSupport += direction * after.mass * (after.vel[2] - before.vel[2]);
    }
  ...
  if (!signal.reactive) throw new Error(`E6.0b ${kind} failed to establish support`);
  const settled = wholeBodyState(organism);
  if (Math.abs(settled.mass - TOTAL_MASS) > 1e-3) {
    throw new Error(`E6.0b ${kind} mass=${settled.mass} does not match 80kg comparison mass`);
  }
  ...
  if (!targetReached) throw new Error(`E6.0b ${kind} failed to reach platform target`);
  ...
  const result = {
    ...
    outcome,
    mass: settled.mass,
    launchTiltDeg: organism.torsoTilt * 0 + 0,
    ...
    speedRetention: direction * rampEnd.vel[2] / TARGET_SPEED,
    rampSupportLossFrames,
    maxFootRelativeDrift,
    launchComY: launch.pos[1],
    rampEndComY: rampEnd.pos[1],
  };
...
if (
  ...
  throw new Error('E6.0b expected current Donor-v1/E5 substrate changed; requalify representation match');
}
...
for (const direction of [-1, 1]) {
  const reference = runCase({ kind: 'reference', direction });
  const locked = runCase({ kind: 'locked', direction });
  results.push({ direction, reference, locked });
  console.log(
...
for (const { direction, reference, locked } of results) {
  if (reference.outcome !== 'RECOVER') {
    throw new Error(`E6.0b reference no longer reproduces E5 lead8 RECOVER for dir=${direction}`);
  }
  if (locked.outcome !== 'RECOVER') {
    throw new Error(`E6.0b locked three-body control does not preserve recoverability for dir=${direction}`);
  }
  if (locked.rampSupportLossFrames !== 0) {
    throw new Error(`E6.0b locked three-body control loses support during ramp for dir=${direction}`);
  }
  if (Math.abs(locked.deliveredVsRequired - reference.deliveredVsRequired) > 0.05) {
    throw new Error(`E6.0b locked support impulse differs too much from E5 reference for dir=${direction}`);
  }
  if (Math.abs(locked.bodySpeedAtRampEnd - reference.bodySpeedAtRampEnd) > 0.25) {
    throw new Error(`E6.0b locked ramp-end speed differs too much from E5 reference for dir=${direction}`);
  }
  if (Math.abs(locked.peakTiltDeg - reference.peakTiltDeg) > 4) {
    throw new Error(`E6.0b locked peak tilt differs too much from E5 reference for dir=${direction}`);
  }
}

const lockedMirrorSpeed = Math.abs(results[0].locked.bodySpeedAtRampEnd - results[1].locked.bodySpeedAtRampEnd);
const lockedMirrorImpulse = Math.abs(results[0].locked.deliveredVsRequired - results[1].locked.deliveredVsRequired);
if (lockedMirrorSpeed > 0.15 || lockedMirrorImpulse > 0.035) {
  throw new Error('E6.0b locked three-body control is not sufficiently mirrored for an actuator causality test');
}

console.log('E6.0b PASS: a locked three-body slider representation preserves the qualified E5 current31/lead8 control closely enough to isolate the slider actuator next. No support-relative translation has been enabled yet, so this is a representation gate rather than evidence for relocation.');
